...
class PermisosController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $permisos = Permission::orderBy('id', 'asc')
                                ->where(function($query){
                                    if(request()->input('queryPermisos')){
                                        $searchValue = strtolower(request()->input('queryPermisos'));
                                        $query->whereRaw('LOWER(permissions.name) LIKE ?', ["%{$searchValue}%"]);
                                    }
                                });
    ...
    public function store(Request $request){
        ...
        $permiso = Permission::where('name', 'LIKE', '%'. $request->name)->pluck('id')->first();

        $var = Permission::firstOrCreate(['name' => $request->nombre, 'guard_name' => 'web']);
