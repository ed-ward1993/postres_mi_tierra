...
export default function Usuario({errors}) {
    ...
    const openModal = (op, id, nombres, apellidos, tipo_documento, numero_documento, usuario, roles,  email, contrasena, observaciones, persona_id) => {
        setFormulario(true);
        setListado(false);
        setOperation(op);
        setData({
            nombres: '', apellidos: '', tipo_documento: 0, numero_documento: '', usuario: '', roles: 0, email: '', contrasena: '', observaciones: ''
        });
        if (op === 1) {
        }
        else {
            getSedes(empresas);
            setData({
                id: id, nombres: nombres, apellidos: apellidos, tipo_documento: tipo_documento,
                numero_documento: numero_documento, usuario: usuario, roles: roles, email: email, contrasena: '', observaciones: observaciones,persona_id: persona_id
            });
        }
    }
    ...
    const save = (e) => {
        e.preventDefault();
                     if (operation === 1) {
                    post(route("usuarios.store"),{preserveState: true, preserveScroll: true, onSuccess: function (response) {
                        Swal.fire({
                            icon: "success",
            ...
            post(route("usuarios.update"),{
                ...
                id_user_empresa_sede: data.id_user_empresa_sede,
    ...
    const getSelectedEmpresas = (selectedOption) => {
        setData("empresas", selectedOption.id);
        getSedes(selectedOption.id);
    }

    const getSelectedSedes = (selectedOption) => {
        setData("sedes", selectedOption.id);
    }

    const getSedes = (id) => {
        axios.post(route('usuarios.sedes'), { id_empresa: id }).then((response) => {
            global = response.data;
            setListadoSedes(global);
        }).catch((error) => {
                                                ...
                                                {(data.roles === 4) && (
                                                    <div className="px-3  lg:col-span-3 col-span-6">
                                                        ...
                                                        <DynamicSelect
                                                            multiple={false}
                                                            withIcons={true}
                                                            options={listadoSedes}
                                                        errors={errors.sedes}
                                                        value={data.sedes}
                                                            valueKey="id"
                                                            labelKey="nombre"
                                                            onChange={getSelectedSedes}
                                                        />
                                                    </div>
                                                )}
                                                </>
